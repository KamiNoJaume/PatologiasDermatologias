...
(async () => {
  ...
  await page.screenshot({path: 'screenshot_branch_b_colors.png'});
  
  // Go back to question
  await page.click('.nav-buttons-row .btn-start-flow'); 
  await new Promise(r => setTimeout(r, 500));
  
  // Click Branch C
  const answers2 = await page.$$('.option-button');
  await answers2[2].click();
  await new Promise(r => setTimeout(r, 500));
  
  await page.screenshot({path: 'screenshot_branch_c_colors.png'});
